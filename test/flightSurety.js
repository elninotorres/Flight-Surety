...
contract('FlightSurety Tests', async (accounts) => {
    ...
    it(`Initial isOperational() value`, async function () {
    ...
    it(`First airline is registered when contract is deployed`, async function () {
        let wasRegistered = (
            await config.flightSuretyData.hasAirlineBeenRegistered.call(
                firstAirline));
        assert.equal(wasRegistered, true, "First airline not registered on deploy");

        let wasFunded = (
            await config.flightSuretyData.hasFundingBeenSubmitted.call(
                firstAirline));
        assert.equal(wasFunded, true, "First airline should be funded on deploy");
    });
    ...
    it(`Only existing airline may register a new airline until there are at least four airlines registered`, async function () {
        ...
        assert.notEqual(errorThrown, undefined,'Revert error not thrown for registering without firstAirline');
        assert.isAbove(
